...
const CompanySetup = () => {
    // Core hooks for routing, data fetching, and navigation
    const params = useParams();
    useGetCompanyById(params.id); // Custom hook to fetch company data
    ...
    // Handler specifically for file input
    const changeFileHandler = (e) => {
        const file = e.target.files?.[0];
        setInput({ ...input, file });
    };

    // Main logic: Handle form submission to update company details
    const submitHandler = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                navigate("/admin/companies"); // Redirect on success
            }
        } catch (error) {
    ...
    // Effect to pre-fill the form once company data is fetched and available in Redux
    useEffect(() => {
        if (singleCompany) {
            setInput({
        ...
        <div>
            ...
            <div className="max-w-xl mx-auto my-10">
                <form onSubmit={submitHandler}>
                    ...
                    {/* Form fields are bound to the 'input' state */}
                    ...
                    {/* Conditional rendering for the submit button to show a loading state */}
                    {loading ? (
