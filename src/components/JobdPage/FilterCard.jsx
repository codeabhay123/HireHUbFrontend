...
const FilterCard = () => {
  ...
  return (
    <div className="w-full bg-white-50 p-6 rounded-lg shadow-md font-sans">
      ...
      <RadioGroup value={selectedValue} onValueChange={changeHandler}>
        {filterData.map((data, index) => (
          <div key={index} className="mb-6">
            <h2 className="font-semibold text-md mb-3 text-gray-800">{data.filterType}</h2>
            <div className="space-y-2">
              {data.array.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <RadioGroupItem value={item} id={itemId} className="ring-offset-white focus:ring-blue-500" />
                    <Label htmlFor={itemId} className={`cursor-pointer select-none ${isSelected ? "font-semibold text-blue-700" : "text-gray-700"}`}>
                      {item}
                    </Label>
                  </div>
                )
